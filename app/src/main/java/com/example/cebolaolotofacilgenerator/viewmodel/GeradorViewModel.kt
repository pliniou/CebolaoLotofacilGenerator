...
class GeradorViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun validarConfiguracoes(): Pair<Boolean, String?> {
        ...
        if (quantidadeJogos <= 0) {
            return Pair(false, "Quantidade de jogos deve ser maior que zero")
        }
        
        // Verificar quantidade de números por jogo
        if (quantidadeNumeros !in 15..20) {
            return Pair(false, "Quantidade de números deve estar entre 15 e 20")
        }
        
        // Verificar se há números fixos demais
        if (numerosFixos.size > quantidadeNumeros) {
            return Pair(false, "Quantidade de números fixos não pode ser maior que a quantidade de números por jogo")
        }
        
        // Verificar se há números excluídos demais
        if (25 - numerosExcluidos.size < quantidadeNumeros) {
            return Pair(false, "Muitos números excluídos, impossível gerar jogos com a quantidade solicitada")
        }
        
        // Verificar sobreposição entre fixos e excluídos
        val sobreposicao = verificarSobreposicao()
        if (sobreposicao.isNotEmpty()) {
            return Pair(false, "Há números que estão tanto na lista de fixos quanto na de excluídos: ${sobreposicao.sorted()}")
        }
     ...
     */
    fun gerarJogos() = viewModelScope.launch {
        try {
            // Valida as configurações
            val (valido, _) = validarConfiguracoes() // Usando _ para ignorar o erro não utilizado
            if (!valido) {
