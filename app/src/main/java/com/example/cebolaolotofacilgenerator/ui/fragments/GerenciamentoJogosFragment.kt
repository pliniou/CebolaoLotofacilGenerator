...
class GerenciamentoJogosFragment : Fragment() {
    ...
    private fun setupRecyclerView() {
        jogosAdapter = JogosGerenciamentoAdapter(
    ...
    private fun setupObservers() {
        ...
        viewModel.todosJogos.observe(viewLifecycleOwner) { jogos ->
    ...
    private fun setupListeners() {
        ...
        binding.tabLayout.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) {
                when (tab.position) {
                    ...
                    1 -> { // Favoritos
                        viewModel.jogosFavoritos.observe(viewLifecycleOwner) { jogos ->
                            jogosAdapter.submitList(jogos)
                            atualizarVisibilidadeListaVazia(jogos.isEmpty())
                        }
                    }
                    2 -> { // Conferidos
                        viewModel.jogosConferidos.observe(viewLifecycleOwner) { jogos ->
                            jogosAdapter.submitList(jogos)
                            atualizarVisibilidadeListaVazia(jogos.isEmpty())
                        }
                    }
        ...
        binding.buttonLimparTodos.setOnClickListener {
    ...
    private fun confirmarExclusao(jogo: com.example.cebolaolotofacilgenerator.data.modelo.Jogo) {
        AlertDialog.Builder(requireContext())
            .setTitle(R.string.confirmar_exclusao)
            .setMessage(getString(R.string.mensagem_confirmar_exclusao_jogo, jogo.numeros.joinToString(" - ")))
            .setPositiveButton(R.string.excluir) { _, _ ->
                viewModel.deletarJogo(jogo)
                Toast.makeText(requireContext(), R.string.jogo_excluido, Toast.LENGTH_SHORT).show()
            }
    ...
    private fun confirmarLimparTodos() {
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton(R.string.confirmar) { _, _ ->
                ...
                Toast.makeText(requireContext(), R.string.todos_jogos_excluidos, Toast.LENGTH_SHORT).show()
            }
            .setNegativeButton(R.string.cancelar, null)
            .show()
    }
    
    private fun exibirDetalhesJogo(jogo: com.example.cebolaolotofacilgenerator.data.modelo.Jogo) {
        val mensagem = StringBuilder()
            .append(getString(R.string.numeros_jogados, jogo.numeros.joinToString(" - ")))
            .append("\n\n")
            .append(getString(R.string.data_geracao, android.text.format.DateFormat.format("dd/MM/yyyy HH:mm", jogo.dataGeracao)))
            .append("\n\n")
            .append(getString(R.string.caracteristicas_jogo))
            .append("\n")
            .append(getString(R.string.pares_impares, jogo.quantidadePares, jogo.quantidadeImpares))
            .append("\n")
            ...
            .append("\n")
            .append(getString(R.string.miolo_moldura, jogo.quantidadeMiolo, jogo.quantidadeMoldura))
            .append("\n")
