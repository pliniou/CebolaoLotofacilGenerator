...
class ConfiguracoesFragment : Fragment() {
    ...
    private fun setupObservers() {
        viewModel.temaAtual.observe(viewLifecycleOwner) { tema ->
            atualizarDescricaoTema(tema)
        }
    }
    
    private fun setupListeners() {
        binding.cardViewTema.setOnClickListener {
            mostrarDialogoSelecionarTema()
        }
        
        binding.cardViewSobre.setOnClickListener {
            mostrarDialogoSobre()
        }
    }
    
    private fun atualizarDescricaoTema(tema: ConfiguracoesViewModel.TemaAplicativo) {
        val textoTema = when (tema) {
            ...
            ConfiguracoesViewModel.TemaAplicativo.SISTEMA -> getString(R.string.tema_sistema)
        }
