    ...
    companion object {
        ...
        fun gerarJogos(
            ...
            if (quantidadeNumeros !in MIN_NUMEROS..MAX_NUMEROS) {
                throw IllegalArgumentException("Quantidade de números deve estar entre $MIN_NUMEROS e $MAX_NUMEROS")
            }
            ...
            if (numerosFixos.size > quantidadeNumeros) {
                throw IllegalArgumentException("Quantidade de números fixos não pode ser maior que a quantidade de números por jogo")
            }
            
            if (TOTAL_NUMEROS - numerosExcluidos.size < quantidadeNumeros) {
                throw IllegalArgumentException("Muitos números excluídos, impossível gerar jogos com a quantidade solicitada")
            }
            ...
            if (sobreposicao.isNotEmpty()) {
                throw IllegalArgumentException("Há números que estão tanto na lista de fixos quanto na lista de excluídos: $sobreposicao")
            }
            
            val jogosGerados = mutableListOf<Jogo>()
            val tentativasMaximas = quantidadeJogos * 100 // Limite de tentativas para evitar loop infinito
            var tentativas = 0
            ...
            while (jogosGerados.size < quantidadeJogos && tentativas < tentativasMaximas) {
                ...
                try {
                    ...
                    val numerosDisponiveis = (1..TOTAL_NUMEROS)
                    ...
                    // Se não conseguiu completar a quantidade de números, pula para a próxima tentativa
                    ...
                    if (!verificarCriteriosEstatisticos(
                            ...
                            minPares, maxPares,
                            minPrimos, maxPrimos,
                            minFibonacci, maxFibonacci,
                            minMiolo, maxMiolo,
                            minMultiplosTres, maxMultiplosTres,
                            minSoma, maxSoma
                        )) {
                        continue
                    }
                    ...
                    val jogoString = jogoOrdenado.joinToString(",")
                    
                    if (jogosGerados.any { it.numeros == jogoString }) {
                        continue
                    }
        ...
        private fun verificarCriteriosEstatisticos(
            numeros: List<Int>,
            minPares: Int?, maxPares: Int?,
            minPrimos: Int?, maxPrimos: Int?,
            minFibonacci: Int?, maxFibonacci: Int?,
            minMiolo: Int?, maxMiolo: Int?,
            minMultiplosTres: Int?, maxMultiplosTres: Int?,
            minSoma: Int?, maxSoma: Int?
        ): Boolean {
